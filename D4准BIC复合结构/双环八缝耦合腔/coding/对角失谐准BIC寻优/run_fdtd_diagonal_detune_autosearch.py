...
TARGETS = {
    "peak_min": 0.95,
    "peak_max": 1.0,
    "offband_p95_max": 0.015,
    "offband_local_peak_max": 0.015,
...
STRICT_POWER_RUNTIME = {
    "name": "strict_power_mesh2_fast_Tonly",
    "simulation_time_fs": 80000.0,
    "mesh_accuracy": 2,
    "dt_stability_factor": 0.995,
    "auto_shutoff_min": 1e-8,
    "auto_shutoff_max": 100000,
    "down_sample_time": 100,
    "min_mesh_step_um": None,
    "max_source_time_signal_length": 32768,
    "background_index": 1.0,
    "frequency_points": 901,
    "add_reflection_monitor": False,
    "reflection_monitor_z_offset_um": 1.05,
    "local_mesh_enabled": False,
    "local_mesh_dx_nm": 6.0,
    "local_mesh_dy_nm": 6.0,
    "local_mesh_dz_nm": 10.0,
    "local_mesh_margin_nm": 18.0,
}


STRICT_POWER_CANDIDATES = [
    {
        "study_case": "strict_extra115",
        "screen_aperture_extra_length_nm": 115.0,
        "screen_aperture_width_nm": 36.0,
        "gold_thickness_nm": 90.0,
    },
    {
        "study_case": "strict_extra117p5",
        "screen_aperture_extra_length_nm": 117.5,
        "screen_aperture_width_nm": 36.0,
        "gold_thickness_nm": 90.0,
    },
    {
        "study_case": "strict_extra116p25",
        "screen_aperture_extra_length_nm": 116.25,
        "screen_aperture_width_nm": 36.0,
        "gold_thickness_nm": 90.0,
    },
    {
        "study_case": "strict_extra116p9",
        "screen_aperture_extra_length_nm": 116.9,
        "screen_aperture_width_nm": 36.0,
        "gold_thickness_nm": 90.0,
    },
    {
        "study_case": "strict_eta6p4_extra120",
        "eta_nm": 6.4,
        "screen_aperture_extra_length_nm": 120.0,
        "screen_aperture_width_nm": 36.0,
        "gold_thickness_nm": 90.0,
    ...
    {
        "study_case": "strict_eta5p8_extra120",
        "eta_nm": 5.8,
        "screen_aperture_extra_length_nm": 120.0,
        "screen_aperture_width_nm": 36.0,
        "gold_thickness_nm": 90.0,
    ...
    {
        "study_case": "strict_eta0_extra120",
        "eta_nm": 0.0,
        "screen_aperture_extra_length_nm": 120.0,
        "screen_aperture_width_nm": 36.0,
        "gold_thickness_nm": 90.0,
    ...
    {
        "study_case": "strict_eta0_gap10_extra120",
        "eta_nm": 0.0,
        "base_gap_nm": 10.0,
        "screen_aperture_extra_length_nm": 120.0,
        "screen_aperture_width_nm": 36.0,
    ...
    {
        "study_case": "strict_shiftm15_extra120",
        "inner_outer_shift_nm": -15.0,
        "screen_aperture_extra_length_nm": 120.0,
        "screen_aperture_width_nm": 36.0,
        "gold_thickness_nm": 90.0,
    },
    {
        "study_case": "strict_gap18_shiftm15_extra120",
        "base_gap_nm": 18.0,
        "inner_outer_shift_nm": -15.0,
        "screen_aperture_extra_length_nm": 120.0,
        "screen_aperture_width_nm": 36.0,
    ...
    {
        "study_case": "strict_shiftm15_extra120_hiresspec",
        "inner_outer_shift_nm": -15.0,
        "screen_aperture_extra_length_nm": 120.0,
        "screen_aperture_width_nm": 36.0,
        "gold_thickness_nm": 90.0,
        "frequency_points": 2401,
    },
    {
        "study_case": "strict_bridge34_shiftm15_extra120",
        "inner_outer_shift_nm": -15.0,
        "bridge_width_nm": 34.0,
        "screen_aperture_extra_length_nm": 120.0,
        "screen_aperture_width_nm": 36.0,
        "gold_thickness_nm": 90.0,
    },
    {
        "study_case": "strict_ringgap40_shiftm15_extra120",
        "outer_inner_coupling_gap_nm": 40.0,
        "inner_outer_shift_nm": -15.0,
        "screen_aperture_extra_length_nm": 120.0,
        "screen_aperture_width_nm": 36.0,
        "gold_thickness_nm": 90.0,
    ...
    {
        "study_case": "strict_extra128",
        "screen_aperture_extra_length_nm": 128.0,
        "screen_aperture_width_nm": 36.0,
        "gold_thickness_nm": 90.0,
    },
    {
        "study_case": "strict_extra130",
        "screen_aperture_extra_length_nm": 130.0,
        "screen_aperture_width_nm": 36.0,
        "gold_thickness_nm": 90.0,
    },
...
def _rotated_rect_bbox_span(length_m, width_m, angle_deg):
...
def add_mesh_box(fdtd, name, x, y, x_span, y_span, z_min, z_max, dx, dy, dz):
    fdtd.addmesh()
    fdtd.set("name", name)
    fdtd.set("x", x)
    fdtd.set("y", y)
...
def add_low_cost_local_mesh_override(fdtd, config, height):
    if not config.get("local_mesh_enabled"):
        return
    dx = nm_to_m(float(config.get("local_mesh_dx_nm", 6.0)))
    dy = nm_to_m(float(config.get("local_mesh_dy_nm", 6.0)))
    dz = nm_to_m(float(config.get("local_mesh_dz_nm", 10.0)))
    margin = nm_to_m(float(config.get("local_mesh_margin_nm", 18.0)))
    metal_z_min = -nm_to_m(float(config["gold_thickness_nm"])) - 2.0 * NM
    metal_z_max = 2.0 * NM
    si_z_min = -2.0 * NM
    si_z_max = height + 2.0 * NM
    ...
    for angle in (0.0, 45.0, 90.0, 135.0, 180.0, 225.0, 270.0, 315.0):
        ...
        x_span, y_span = _rotated_rect_bbox_span(aperture_length + 2.0 * margin, nm_to_m(width_nm) + 2.0 * margin, angle)
        add_mesh_box(
            fdtd,
            "local_mesh_metal_slit_{:03d}".format(int(angle)),
            x,
            y,
            x_span,
            y_span,
    ...
    outer_radius = nm_to_m(0.5 * (config["outer_inner_radius_nm"] + config["outer_outer_radius_nm"]))
    gap_length = nm_to_m(float(config["outer_ring_width_nm"])) + 2.0 * margin
    gap_width = nm_to_m(max(float(config["base_gap_nm"]) + abs(float(config["eta_nm"])), 10.0)) + 2.0 * margin
    for angle in (0.0, 45.0, 90.0, 135.0, 180.0, 225.0, 270.0, 315.0):
        x, y = polar(outer_radius, angle)
        x_span, y_span = _rotated_rect_bbox_span(gap_length, gap_width, angle + 90.0)
        add_mesh_box(
            fdtd,
            "local_mesh_si_gap_{:03d}".format(int(angle)),
            x,
            y,
            x_span,
            y_span,
...
def add_power_monitor(fdtd, config, monitor_name, z_m):
    fdtd.addpower()
...
def add_single_monitor(fdtd, config, monitor_mode="power", monitor_name=None):
    ...
    if monitor_mode == "field":
        ...
        add_power_monitor(fdtd, config, monitor_name, float(config.get("transmission_monitor_z_um", -0.25)) * UM)
        return monitor_name
    fdtd.set("name", monitor_name)
    fdtd.set("monitor type", "2D Z-normal")
    fdtd.set("x span", nm_to_m(config["period_nm"]))
...
def build_candidate_geometry(fdtd, config, monitor_mode="power", monitor_name=None):
    ...
    if monitor_mode == "power" and config.get("add_reflection_monitor"):
        add_power_monitor(
            fdtd,
            config,
            "R",
            height + float(config.get("reflection_monitor_z_offset_um", 0.45)) * UM,
        )
    ...
    add_low_cost_local_mesh_override(fdtd, config, height)
...
def make_config(candidate, mode):
    ...
    config = derive_base_config(model)
...
def extract_transmission(fdtd, monitor_name="T"):
    wl = None
    tr = None
    try:
    ...
    if wl is None:
        try:
            wl = fdtd.getdata(monitor_name, "lambda")
        except Exception:
            wl = None
    if tr is None:
        tr = fdtd.transmission(monitor_name)
    wl = np.asarray(wl if wl is not None else []).reshape(-1).astype(float)
    tr = np.asarray(tr).reshape(-1)
    if wl.size == 0:
        wl = C0 / np.asarray(fdtd.getdata(monitor_name, "f")).reshape(-1).astype(float)
...
def monitor_frequency(fdtd, monitor_name, fallback_wavelength_m=None):
    try:
        f = np.asarray(fdtd.getdata(monitor_name, "f"), dtype=float).reshape(-1)
        if f.size:
            return f
    except Exception:
        pass
    if fallback_wavelength_m is None:
        return np.asarray([], dtype=float)
    wl = np.asarray(fallback_wavelength_m, dtype=float).reshape(-1)
    wl = wl[np.isfinite(wl) & (wl > 0)]
    return C0 / wl if wl.size else np.asarray([], dtype=float)


def sourcepower_spectrum(fdtd, frequency_hz):
    f = np.asarray(frequency_hz, dtype=float).reshape(-1)
    if f.size == 0:
        return np.asarray([], dtype=float)
    try:
        return np.asarray(fdtd.sourcepower(f), dtype=float).reshape(-1)
    except Exception:
        return np.full(f.shape, np.nan, dtype=float)
...
def _field_component(result, key):
    val = _dataset_get(result, key, None)
    if val is None:
        return None
    return np.asarray(val)


def _reshape_spectrum_array(values, nf):
    arr = np.asarray(values)
    if arr.size == 0 or nf <= 0:
        return None
    if arr.shape[-1] == nf:
        return arr.reshape(-1, nf)
    for axis, size in enumerate(arr.shape):
        if size == nf:
            return np.moveaxis(arr, axis, -1).reshape(-1, nf)
    return None


def _align_vector(values, n):
...
def manual_poynting_flux(fdtd, monitor_name, frequency_hz):
    f = np.asarray(frequency_hz, dtype=float).reshape(-1)
    nf = int(f.size)
    if nf == 0:
        return np.asarray([], dtype=float)
    try:
        e_result = fdtd.getresult(monitor_name, "E")
        h_result = fdtd.getresult(monitor_name, "H")
    except Exception:
        return np.full(nf, np.nan, dtype=float)

    ex = _field_component(e_result, "Ex")
    ey = _field_component(e_result, "Ey")
    hx = _field_component(h_result, "Hx")
    hy = _field_component(h_result, "Hy")
    if ex is None or ey is None or hx is None or hy is None:
        return np.full(nf, np.nan, dtype=float)

    ex2 = _reshape_spectrum_array(ex, nf)
    ey2 = _reshape_spectrum_array(ey, nf)
    hx2 = _reshape_spectrum_array(hx, nf)
    hy2 = _reshape_spectrum_array(hy, nf)
    if ex2 is None or ey2 is None or hx2 is None or hy2 is None:
        return np.full(nf, np.nan, dtype=float)

    x = _dataset_get(e_result, "x", None)
    y = _dataset_get(e_result, "y", None)
    if x is None:
        try:
    ...
    sz = 0.5 * np.real(ex2 * np.conjugate(hy2) - ey2 * np.conjugate(hx2))
    return np.nansum(sz, axis=0).astype(float) * dx * dy


def sign_corrected_power(raw_transmission, monitor_role):
    raw = np.asarray(raw_transmission).reshape(-1)
    real_raw = np.real(raw).astype(float)
    if monitor_role == "T":
        return np.maximum(real_raw, 0.0), "fdtd_transmission_power_raw_real_positive_T"
    if monitor_role == "R":
        return real_raw, "fdtd_reflection_power_upward_sign_corrected"
    return real_raw, "fdtd_transmission_power_raw"


def extract_power_audit(fdtd):
    wl_t, raw_t = extract_transmission(fdtd, "T")
    frequency = monitor_frequency(fdtd, "T", wl_t)
    source_power = sourcepower_spectrum(fdtd, frequency)
    t_power, metric_basis = sign_corrected_power(raw_t, "T")
    audit = {
        "wavelength_m": wl_t,
        "frequency_hz": frequency,
        "T_raw": raw_t,
        "T_power": t_power,
        "T_abs2_legacy": np.abs(np.asarray(raw_t).reshape(-1)) ** 2,
        "sourcepower": source_power,
        "metric_basis": metric_basis,
        "T_poynting_flux_W": manual_poynting_flux(fdtd, "T", frequency),
    }
    source_power_aligned = _align_vector(source_power, audit["T_poynting_flux_W"].size)
    audit["sourcepower"] = source_power_aligned
    if source_power_aligned.size:
        with np.errstate(divide="ignore", invalid="ignore"):
            audit["T_poynting_power"] = -audit["T_poynting_flux_W"] / source_power_aligned
    else:
        audit["T_poynting_power"] = np.full(t_power.shape, np.nan, dtype=float)

    try:
        _, raw_r = extract_transmission(fdtd, "R")
        r_power, _ = sign_corrected_power(raw_r, "R")
        n = min(wl_t.size, raw_r.size, t_power.size)
        audit["R_raw"] = np.asarray(raw_r).reshape(-1)[:n]
        audit["R_power"] = np.asarray(r_power).reshape(-1)[:n]
        audit["R_poynting_flux_W"] = manual_poynting_flux(fdtd, "R", frequency)[:n]
        source_power_r = _align_vector(audit["sourcepower"], audit["R_poynting_flux_W"].size)
        if source_power_r.size:
            with np.errstate(divide="ignore", invalid="ignore"):
                audit["R_poynting_power"] = audit["R_poynting_flux_W"] / source_power_r
        else:
            audit["R_poynting_power"] = np.full(n, np.nan, dtype=float)
        audit["A_from_TR"] = 1.0 - np.asarray(audit["T_power"])[:n] - np.asarray(audit["R_power"])[:n]
        audit["A_from_poynting_TR"] = 1.0 - np.asarray(audit["T_poynting_power"])[:n] - np.asarray(audit["R_poynting_power"])[:n]
    except Exception:
        audit["R_raw"] = np.full(t_power.shape, np.nan, dtype=float)
        audit["R_power"] = np.full(t_power.shape, np.nan, dtype=float)
        audit["R_poynting_flux_W"] = np.full(t_power.shape, np.nan, dtype=float)
        audit["R_poynting_power"] = np.full(t_power.shape, np.nan, dtype=float)
        audit["A_from_TR"] = np.full(t_power.shape, np.nan, dtype=float)
        audit["A_from_poynting_TR"] = np.full(t_power.shape, np.nan, dtype=float)
    return audit


def add_power_audit_metrics(metrics, audit):
    wl_nm = np.asarray(audit.get("wavelength_m", []), dtype=float).reshape(-1) / NM
    if wl_nm.size == 0 or "peak_nm" not in metrics:
        return metrics
    peak_idx = int(np.nanargmin(np.abs(wl_nm - float(metrics["peak_nm"]))))
    for key in (
        "T_power",
        "T_abs2_legacy",
        "R_power",
        ...
        values = np.asarray(audit.get(key, []), dtype=float).reshape(-1)
        if peak_idx < values.size:
            metrics["peak_" + key] = float(values[peak_idx])
    if "A_from_TR" in audit:
        a_vals = np.asarray(audit.get("A_from_TR", []), dtype=float).reshape(-1)
        if a_vals.size and np.any(np.isfinite(a_vals)):
            metrics["min_A_from_TR"] = float(np.nanmin(a_vals))
            metrics["max_energy_sum_TR"] = float(np.nanmax(1.0 - a_vals))
    return metrics
...
def transmission_power_metric(transmission_power):
    power = np.asarray(transmission_power, dtype=float).reshape(-1)
    if power.size == 0:
        return np.asarray([], dtype=float), "empty_power_T"
    return power, "fdtd_transmission_power_raw_real_positive_T"
...
def spectrum_metrics(wavelength_m, transmission):
    power, metric_basis = transmission_power_metric(transmission)
...
def _audit_column(audit, key, n):
    return _align_vector(audit.get(key, []), n)


def save_transmission_xlsx(path, wavelength_m, transmission, candidate, metrics, audit=None):
    wl_nm = np.asarray(wavelength_m).reshape(-1) / NM
    tr = np.asarray(transmission).reshape(-1)
    if audit is None:
        t_power, _ = sign_corrected_power(tr, "T")
        audit = {
            "T_raw": tr,
            "T_power": t_power,
            "T_abs2_legacy": np.abs(tr) ** 2,
        }
    n = min(wl_nm.size, tr.size if tr.size else wl_nm.size)
    rows = [
        [
            "wavelength_nm",
            "T_power_raw_real_positive_acceptance",
            "T_raw_real",
            "T_raw_imag",
            "T_abs2_legacy_not_for_acceptance",
            "R_power_sign_corrected",
            "R_raw_real",
            "R_raw_imag",
            "A_from_1_minus_T_minus_R",
            "sourcepower_W",
            "T_poynting_power",
            "R_poynting_power",
            "A_from_poynting_TR",
    ...
    t_raw = np.asarray(audit.get("T_raw", tr)).reshape(-1)
    r_raw = np.asarray(audit.get("R_raw", np.full(n, np.nan))).reshape(-1)
    for i in range(n):
        t_val = t_raw[i] if i < t_raw.size else complex(float("nan"), 0)
        r_val = r_raw[i] if i < r_raw.size else complex(float("nan"), 0)
        rows.append(
            [
                float(wl_nm[i]),
                float(_audit_column(audit, "T_power", n)[i]),
                float(np.real(t_val)),
                float(np.imag(t_val)),
                float(_audit_column(audit, "T_abs2_legacy", n)[i]),
                float(_audit_column(audit, "R_power", n)[i]),
                float(np.real(r_val)),
                float(np.imag(r_val)),
                float(_audit_column(audit, "A_from_TR", n)[i]),
                float(_audit_column(audit, "sourcepower", n)[i]),
                float(_audit_column(audit, "T_poynting_power", n)[i]),
                float(_audit_column(audit, "R_poynting_power", n)[i]),
                float(_audit_column(audit, "A_from_poynting_TR", n)[i]),
                float(_audit_column(audit, "T_poynting_flux_W", n)[i]),
                float(_audit_column(audit, "R_poynting_flux_W", n)[i]),
            ]
        )
    meta = [["key", "value"]]
    ...
    save_xlsx(path, [("power_audit", rows), ("metadata", meta)])


def save_plot(path, wavelength_m, transmission, candidate, metrics, audit=None):
    try:
        import matplotlib
        ...
        return False
    wl_nm = np.asarray(wavelength_m).reshape(-1) / NM
    if audit is not None and "T_power" in audit:
        power = np.asarray(audit["T_power"], dtype=float).reshape(-1)
    else:
        power, _ = transmission_power_metric(transmission)
    fig, ax = plt.subplots(figsize=(8.6, 5.2), dpi=170)
    ax.plot(wl_nm, power, linewidth=1.6, color="#155e75")
    ax.set_xlabel("Wavelength (nm)")
    ax.set_ylabel("Power transmission T")
    ...
    if "peak_nm" in metrics:
        ...
        ax.text(
            ...
            "peak T={:.4g} @ {:.2f} nm\nFWHM={:.3g} nm\np95 offband={:.4g}".format(
                metrics.get("peak_T", metrics.get("peak_abs2", float("nan"))),
                metrics.get("peak_nm", float("nan")),
                metrics.get("fwhm_nm", float("nan")),
                metrics.get("offband_p95_T", metrics.get("offband_p95_abs2", float("nan"))),
            ),
...
def run_candidate(lumapi, candidate, mode, folders, runtime_overrides=None, monitor_mode="power"):
    ...
    xlsx_path = folders["excel"] / (stem + "_power_audit.xlsx")
    png_path = folders["png"] / (stem + "_power_T.png")
    ...
    try:
        ...
        if monitor_mode == "field":
            ...
            power_audit = extract_power_audit(fdtd)
            wavelength_m = np.asarray(power_audit["wavelength_m"], dtype=float).reshape(-1)
            transmission = np.asarray(power_audit["T_power"], dtype=float).reshape(-1)
            metrics = spectrum_metrics(wavelength_m, transmission)
            add_power_audit_metrics(metrics, power_audit)
        fdtd.save(str(fsp_path))
    ...
    if monitor_mode == "field":
        ...
        if wavelength_m is not None and transmission is not None:
            save_transmission_xlsx(xlsx_path, wavelength_m, transmission, candidate, metrics, audit=power_audit)
            save_plot(png_path, wavelength_m, transmission, candidate, metrics, audit=power_audit)
...
def manifest_fields():
    return [
        ...
        "peak_R_power",
        "peak_A_from_TR",
        "peak_T_poynting_power",
        "peak_R_poynting_power",
        "peak_A_from_poynting_TR",
        "peak_sourcepower",
        "min_A_from_TR",
        "max_energy_sum_TR",
        "contrast_vs_p95",
...
def parse_args():
    ...
    parser.add_argument("--study", choices=["search", "validation", "optimize", "field", "strict_power"], default="search")
    ...
    parser.add_argument("--case", default=None, help="Run only one named study_case for staged strict validation.")
...
def stage_acceptance_text(metrics):
    if not metrics:
        return "no metrics"
    checks = [
        "status={}".format(metrics.get("status", "")),
        "accepted={}".format(metrics.get("accepted", "")),
        "peak_T={}".format(metrics.get("peak_T", "")),
        "peak_nm={}".format(metrics.get("peak_nm", "")),
        "fwhm_nm={}".format(metrics.get("fwhm_nm", "")),
        "offband_p95_T={}".format(metrics.get("offband_p95_T", "")),
        "peak_R_power={}".format(metrics.get("peak_R_power", "")),
        "peak_A_from_TR={}".format(metrics.get("peak_A_from_TR", "")),
        "max_energy_sum_TR={}".format(metrics.get("max_energy_sum_TR", "")),
        "peak_T_abs2_legacy={}".format(metrics.get("peak_T_abs2_legacy", "")),
    ]
    return "\n".join("- " + item for item in checks)


def write_stage_report(report_path, stage_name, candidate, metrics, command_line, reason, next_step):
    failure = "none"
    if metrics.get("error"):
        failure = metrics.get("error")
    elif metrics.get("peak_T", 0) > TARGETS.get("peak_max", 1.0):
        failure = "peak_T is larger than 1, reject until normalization and energy checks explain it"
    elif metrics.get("status") == "rejected_oscillatory_background":
        failure = "background has oscillatory or extra local peaks"
    elif not metrics.get("accepted"):
        failure = "strict target not fully satisfied"
    lines = [
        "# Stage report - {}".format(stage_name),
        "",
        "## 修改文件",
        ...
        "```python",
        "power_audit = extract_power_audit(fdtd)",
        "T_power = max(real(transmission('T')), 0)  # T-only acceptance metric after raw-sign audit",
        "T_abs2_legacy = abs(transmission('T'))**2  # diagnostic only, not acceptance",
        "```",
        "",
        "## 运行命令",
        "`{}`".format(command_line),
...
def strict_power_batch_summary(run_dir, outputs):
    summary = {
        "study": "strict_power",
        "purpose": "audit sourcepower-normalized T/R, manual Poynting flux, and R+T+A before accepting the D4 qBIC peak",
        "runtime": STRICT_POWER_RUNTIME,
        "outputs": outputs,
    }
    write_json(run_dir / "04_logs" / "strict_power_summary.json", summary)


def run_strict_power_suite(lumapi, base_candidate, mode, root_run_dir, only_case=None):
    outputs = []
    manifest_path = root_run_dir / "04_logs" / "manifest.csv"
    command_line = "python {} {}".format(Path(__file__).resolve(), " ".join(sys.argv[1:]))
    selected = [row for row in STRICT_POWER_CANDIDATES if only_case in (None, row["study_case"])]
    if only_case is not None and not selected:
        raise RuntimeError("Unknown strict_power case: {}".format(only_case))
    for idx, update in enumerate(selected, start=1):
        ...
        print("[strict_power {}/{}] {}".format(idx, len(selected), case_name))
        runtime_overrides = dict(STRICT_POWER_RUNTIME)
        if "frequency_points" in update:
            runtime_overrides["frequency_points"] = int(update["frequency_points"])
        metrics = run_candidate(
            lumapi,
            candidate,
            mode,
        ...
        report_path = folders["reports"] / "{}_stage_report.md".format(case_name)
        next_step = "If sourcepower-normalized T<=1 and T-plane Poynting power is stable, run convergence variations; otherwise diagnose normalization or monitor placement."
        write_stage_report(
            report_path,
            case_name,
            candidate,
            metrics,
            command_line,
            "Replace GUI Abs(T)^2 acceptance with T-only sourcepower-normalized transmission and T-plane Poynting audit.",
            next_step,
        )
        outputs.append({"candidate": candidate, "metrics": metrics, "run_dir": str(case_dir), "report": str(report_path)})
    strict_power_batch_summary(root_run_dir, outputs)
    root_report = root_run_dir / "08_reports" / "strict_power_phase_summary.md"
    best = None
    for output in outputs:
        metrics = output.get("metrics", {})
        if best is None or float(metrics.get("score", -1e9)) > float(best.get("metrics", {}).get("score", -1e9)):
            best = output
    write_stage_report(
        root_report,
        "strict_power_phase_summary",
        best.get("candidate", {}) if best else {},
        best.get("metrics", {}) if best else {},
        command_line,
        "Summarize strict T-only power audit.",
        "Use the best stable candidate for convergence variations and mid-plane field monitor only after T normalization is physically consistent.",
    )
    return outputs
...
def main():
    ...
    if args.study == "strict_power":
        run_dir = prepare_run_dir("strict_power", args.run_dir)
        folders = ensure_folders(run_dir)
        with single_fdtd_session_guard(run_dir):
            lumapi = import_lumapi()
            seed_path = seed_fsp_path()
            if not seed_path.exists():
                seed_path = save_seed_fsp(lumapi)
            print("Seed FSP referenced: {}".format(seed_path))
            ...
            run_strict_power_suite(lumapi, base_candidate, mode, run_dir, only_case=args.case)
            print("Strict power audit completed: {}".format(run_dir))
            return 0
