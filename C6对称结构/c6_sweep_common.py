...
目录规则沿用前面所有脚本：
- 入口脚本放在 母结构/coding/扰动名/run_*.py；
- 结果放在 母结构/results/扰动名/run_模式_时间戳/；
- 源 fsp 文件夹内的 .fsp 永不写入，只复制。
"""
...
def assert_source_unchanged(source_fsp, expected_hash):
    if file_sha256(source_fsp) != expected_hash:
        raise RuntimeError("源 FSP 文件发生变化，为避免误改源文件，脚本停止：{}".format(source_fsp))
...
def find_source_fsp(structure_root):
    ...
    # 六臂星存在两个 fsp 时，优先使用时间戳最新的 232913。
...
def apply_point(fdtd, config, point):
    ...
    if kind == "set_radius":
        ...
        # 单点缺口：在六裂缝环右侧插入一个 etch 矩形切口，尺寸随 cut 扫描。
        outer = float(getnamed(fdtd, "Si_outer_ring", "radius"))
...
def run(config):
    ...
    for idx, point in enumerate(points, 1):
        ...
        print("    完成，用时 {}；预计剩余 {}".format(format_duration(elapsed), format_duration(eta)))
        if summary.get("max") is not None:
            print("    本点谱信息: max |T|^2={:.6g} @ {:.3f} nm；min |T|^2={:.6g} @ {:.3f} nm".format(summary["max"], summary["max_nm"], summary["min"], summary["min_nm"]))
        rows.append({
            "index": point["index"], "name": point["name"], "status": "done",
            "fsp": str(paths["fsp"]), "xlsx": str(paths["xlsx"]), "png": str(paths["png"]),
            "elapsed_s": "{:.3f}".format(elapsed),
            "max_abs2": "" if summary.get("max") is None else "{:.18e}".format(summary["max"]),
            "max_wavelength_nm": "" if summary.get("max") is None else "{:.9f}".format(summary["max_nm"]),
            "min_abs2": "" if summary.get("max") is None else "{:.18e}".format(summary["min"]),
            "min_wavelength_nm": "" if summary.get("max") is None else "{:.9f}".format(summary["min_nm"]),
        })
