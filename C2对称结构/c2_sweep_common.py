...
def apply_point(fdtd, config, point):
    ...
    if kind == "set_radius":
        ...
        # 扫描“半径差”：被选中的对象从母版半径减少 v，未选对象保持母版值。
        for idx in config["TARGET_INDICES"]:
        ...
        # 扫描“高度差”：被选中对象的 z max 从母版高度降低 v，z min 保持不变。
        for idx in config["TARGET_INDICES"]:
        ...
        if v > 0:
            for idx in config["TARGET_INDICES"]:
                ...
                fdtd.set("x", x0)
                # 放在单个脊的顶部边缘附近，形成局部顶槽。
                fdtd.set("y", y0 + ys / 2.0 - float(config.get("SLOT_DEPTH_NM", 35.0)) * 1e-9 / 2.0)
    ...
    for idx, point in enumerate(points, 1):
        ...
        for load_attempt in range(1, 4):
            try:
                ...
                if load_attempt >= 3:
                    raise
                print("    FDTD 启动/加载失败，准备重试 {}/3：{}".format(load_attempt + 1, e))
                time.sleep(3)
        try:
