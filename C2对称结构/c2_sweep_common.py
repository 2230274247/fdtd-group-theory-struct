    ...
    if kind == "set_radius":
        for idx in config["TARGET_INDICES"]:
            ...
            try:
                ...
                pass
    elif kind == "set_radius_delta":
        # 鎵弿鈥滃崐寰勫樊鈥濓細琚€変腑鐨勫璞′粠姣嶇増鍗婂緞鍑忓皯 v锛屾湭閫夊璞′繚鎸佹瘝鐗堝€笺€?        for idx in config["TARGET_INDICES"]:
            base = float(getnamed(fdtd, obj, "radius", idx))
            new_radius = max(float(config.get("MIN_RADIUS_NM", 5.0)) * 1e-9, base - v)
            setnamed(fdtd, obj, "radius", new_radius, idx)
            try:
                setnamed(fdtd, obj, "radius 2", new_radius, idx)
            except Exception:
                pass
    elif kind == "set_x_span":
        for idx in config["TARGET_INDICES"]:
            setnamed(fdtd, obj, "x span", v, idx)
    elif kind == "set_x_span_delta":
        for idx in config["TARGET_INDICES"]:
            base = float(getnamed(fdtd, obj, "x span", idx))
            setnamed(fdtd, obj, "x span", max(float(config.get("MIN_X_SPAN_NM", 5.0)) * 1e-9, base - v), idx)
    elif kind == "set_y_span":
        for idx in config["TARGET_INDICES"]:
            setnamed(fdtd, obj, "y span", v, idx)
    elif kind == "set_y_span_delta":
        for idx in config["TARGET_INDICES"]:
            base = float(getnamed(fdtd, obj, "y span", idx))
            setnamed(fdtd, obj, "y span", max(float(config.get("MIN_Y_SPAN_NM", 5.0)) * 1e-9, base - v), idx)
        ...
        axis = config.get("OFFSET_AXIS", "radial")
        if axis == "x":
            setnamed(fdtd, obj, "x", x0 + v, idx)
        elif axis == "y":
            setnamed(fdtd, obj, "y", y0 + v, idx)
        else:
            x, y = radial_offset(x0, y0, v)
            setnamed(fdtd, obj, "x", x, idx)
            setnamed(fdtd, obj, "y", y, idx)
    elif kind == "set_pair_spacing_x":
        left_idx, right_idx = config["TARGET_INDICES"]
        setnamed(fdtd, obj, "x", -v / 2.0, left_idx)
        setnamed(fdtd, obj, "x", v / 2.0, right_idx)
    elif kind == "set_rotation_delta":
        for idx in config["TARGET_INDICES"]:
            base = float(getnamed(fdtd, obj, "rotation 1", idx))
            setnamed(fdtd, obj, "rotation 1", base + v, idx)
    elif kind == "set_height_delta":
        # 鎵弿鈥滈珮搴﹀樊鈥濓細琚€変腑瀵硅薄鐨?z max 浠庢瘝鐗堥珮搴﹂檷浣?v锛寊 min 淇濇寔涓嶅彉銆?        for idx in config["TARGET_INDICES"]:
            zmin = float(getnamed(fdtd, obj, "z min", idx))
            zmax = float(getnamed(fdtd, obj, "z max", idx))
            min_h = float(config.get("MIN_HEIGHT_NM", 20.0)) * 1e-9
            setnamed(fdtd, obj, "z max", max(zmin + min_h, zmax - v), idx)
    elif kind == "insert_hole":
        if v > 0:
            for idx in config["TARGET_INDICES"]:
                x0 = float(getnamed(fdtd, obj, "x", idx))
                y0 = float(getnamed(fdtd, obj, "y", idx))
                zmin = float(getnamed(fdtd, obj, "z min", idx)) - 1e-9
                zmax = float(getnamed(fdtd, obj, "z max", idx)) + 1e-9
                fdtd.addcircle()
                fdtd.set("name", config.get("INSERT_NAME", "air_hole_perturbation"))
                ...
                try:
                    ...
                    pass
    elif kind == "insert_top_slot":
        if v > 0:
            for idx in config["TARGET_INDICES"]:
                x0 = float(getnamed(fdtd, obj, "x", idx))
                y0 = float(getnamed(fdtd, obj, "y", idx))
                xs = float(getnamed(fdtd, obj, "x span", idx))
                ys = float(getnamed(fdtd, obj, "y span", idx))
                zmin = float(getnamed(fdtd, obj, "z min", idx)) - 1e-9
                zmax = float(getnamed(fdtd, obj, "z max", idx)) + 1e-9
                fdtd.addrect()
                fdtd.set("name", config.get("INSERT_NAME", "air_slot_perturbation"))
                fdtd.set("material", "etch")
                fdtd.set("x", x0)
                # 鏀惧湪鍗曚釜鑴婄殑椤堕儴杈圭紭闄勮繎锛屽舰鎴愬眬閮ㄩ《妲姐€?                fdtd.set("y", y0 + ys / 2.0 - float(config.get("SLOT_DEPTH_NM", 35.0)) * 1e-9 / 2.0)
                fdtd.set("x span", min(max(v, 1e-12), xs))
                fdtd.set("y span", float(config.get("SLOT_DEPTH_NM", 35.0)) * 1e-9)
                fdtd.set("z min", zmin)
                fdtd.set("z max", zmax)
                try:
                    fdtd.set("override mesh order from material database", 1)
