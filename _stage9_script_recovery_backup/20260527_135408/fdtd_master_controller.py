...
DEFAULT_RUN_STYLE = "sequential"        # ask / sequential / parallel
...
SCAN_RANGE_OVERRIDE_KEY = "__scan_range__"
SCAN_START_WORDS = ("START", "BEGIN", "MIN")
...
def _literal_config_from_node(node):
    value = _try_literal_eval(node)
    if isinstance(value, dict):
        return value
    if isinstance(node, ast.Call):
        ...
        if func_name == "dict":
            ...
            for kw in node.keywords:
                if kw.arg is None:
                    continue
                kw_value = _try_literal_eval(kw.value)
                if isinstance(kw_value, (int, float, str, bool)):
                    data[str(kw.arg)] = kw_value
            return data
    return None


def collect_literal_settings_from_text(script_text):
    ...
    for node in getattr(tree, "body", []):
        ...
        if isinstance(value, (int, float, str, bool)):
            settings[name] = {
                "key": name,
                "canon": _override_canon_key(name),
                "value": value,
                "source": "assignment",
            }
        if name == "CONFIG":
            cfg = _literal_config_from_node(node.value)
            if isinstance(cfg, dict):
                for k, v in cfg.items():
                    ...
                    settings[raw_key] = {
                        "key": raw_key,
                        "canon": _override_canon_key(raw_key),
                        "value": v,
                        "source": "config",
                    }
    return settings
...
def _entry_by_canon(settings):
    data = OrderedDict()
    for _, item in settings.items():
        data[item["canon"]] = item
    return data


def _candidate_end_keys(prefix, unit):
    out = []
    for word in SCAN_END_WORDS:
        if prefix and unit:
            out.append("%s_%s_%s" % (prefix, word, unit))
        elif prefix:
            out.append("%s_%s" % (prefix, word))
        elif unit:
            out.append("%s_%s" % (word, unit))
        else:
            out.append(word)
    return out


def _candidate_step_keys(prefix, unit):
    out = []
    for word in SCAN_STEP_WORDS:
        if prefix and unit:
            out.append("%s_%s_%s" % (prefix, word, unit))
        elif prefix:
            out.append("%s_%s" % (prefix, word))
...
def detect_scan_groups_from_settings(settings):
    entries = _entry_by_canon(settings)
    groups = []
    seen = set()
    ...
    for canon_key, item in entries.items():
        ...
        start_word = None
        start_index = None
        for word in SCAN_START_WORDS:
            if word in parts:
                start_word = word
                start_index = parts.index(word)
                break
        if start_word is None:
            continue
        prefix = "_".join(parts[:start_index])
        suffix = parts[start_index + 1:]
        unit = suffix[-1] if suffix and suffix[-1] in SCAN_UNITS else ""
        end_key = None
        for cand in _candidate_end_keys(prefix, unit):
            if cand in entries:
                end_key = cand
                break
        step_key = None
        for cand in _candidate_step_keys(prefix, unit):
            if cand in entries:
                step_key = cand
                break
...
def choose_primary_scan_group(groups):
    if not groups:
        return None
    for unit in ("NM", "UM", "M", "RAW"):
        for group in groups:
            if group.get("unit") == unit:
                return group
    return groups[0]


def _coerce_scan_range_object(scan_range):
    if not isinstance(scan_range, dict):
        return {}
    out = {}
    for logical, names in (
        ("start", ("start", "start_nm", "START", "START_NM")),
        ("end", ("end", "end_nm", "stop", "stop_nm", "END", "END_NM", "STOP", "STOP_NM")),
        ("step", ("step", "step_nm", "STEP", "STEP_NM")),
    ):
        for name in names:
            if name in scan_range and scan_range.get(name) is not None:
                out[logical] = scan_range.get(name)
                break
    return out
...
def normalize_overrides_for_script_text(script_text, replacements, script_path=""):
    ...
    if SCAN_RANGE_OVERRIDE_KEY in replacements:
        scan_request.update(_coerce_scan_range_object(replacements.get(SCAN_RANGE_OVERRIDE_KEY)))

    generic_start_keys = ("START_NM", "START", "SCAN_START_NM", "scan_start_nm")
    generic_end_keys = ("END_NM", "STOP_NM", "END", "STOP", "SCAN_STOP_NM", "SCAN_END_NM", "scan_end_nm", "scan_stop_nm")
    generic_step_keys = ("STEP_NM", "STEP", "SCAN_STEP_NM", "scan_step_nm")
    for key in generic_start_keys:
        if key in replacements and replacements.get(key) is not None:
            scan_request.setdefault("start", replacements.get(key))
            break
    for key in generic_end_keys:
        if key in replacements and replacements.get(key) is not None:
            scan_request.setdefault("end", replacements.get(key))
            break
    for key in generic_step_keys:
        if key in replacements and replacements.get(key) is not None:
            scan_request.setdefault("step", replacements.get(key))
            break

    skip_keys = set([SCAN_RANGE_OVERRIDE_KEY])
    skip_keys.update(generic_start_keys)
    skip_keys.update(generic_end_keys)
    skip_keys.update(generic_step_keys)
    for key, value in replacements.items():
        if key not in skip_keys:
            normalized[key] = value

    if scan_request:
        settings = collect_literal_settings_from_text(script_text)
        groups = detect_scan_groups_from_settings(settings)
        group = choose_primary_scan_group(groups)
        if group:
            if "start" in scan_request:
                normalized[group["start_key"]] = scan_request["start"]
            if "end" in scan_request:
                normalized[group["end_key"]] = scan_request["end"]
            if "step" in scan_request:
                normalized[group["step_key"]] = scan_request["step"]
            debug.append("override scan range resolved for %s: %s/%s/%s" % (
                script_path,
                group["start_key"],
                group["end_key"],
                group["step_key"],
            ...
            if len(groups) > 1:
                debug.append("warning: multiple scan groups detected; primary group was used: %r" % group)
        else:
            debug.append("warning: no scan group detected in script; fallback to generic START_NM/END_NM/STEP_NM: %s" % script_path)
            if "start" in scan_request:
                normalized["START_NM"] = scan_request["start"]
            if "end" in scan_request:
                normalized["END_NM"] = scan_request["end"]
            if "step" in scan_request:
                normalized["STEP_NM"] = scan_request["step"]
    return normalized, debug


def _config_aliases_for_key(name):
    canon = _override_canon_key(name)
    lower = canon.lower()
    aliases = OrderedDict()
    ...
    add(canon)
    add(lower)
    add(lower.replace("__", "_"))
    if canon == "START_NM":
        add("start_nm")
        add("scan_start_nm")
    elif canon in ("END_NM", "STOP_NM"):
        add("end_nm")
        add("stop_nm")
        add("scan_end_nm")
        add("scan_stop_nm")
...
def replace_assignments(text, replacements):
    ...
    for name, value in replacements.items():
        ...
        config_keys = _config_aliases_for_key(name)
...
def prepare_script_for_run(item, run_root, overrides):
    ...
    for line in debug:
        print(line)
    text = replace_assignments(text, normalized_repl)
