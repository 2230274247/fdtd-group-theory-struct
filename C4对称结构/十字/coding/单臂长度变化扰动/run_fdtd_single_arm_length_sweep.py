# -*- coding: utf-8 -*-
"""
Si 十字扰动 3：单臂长度变化 FDTD 自动化扫描脚本
================================================

本脚本实现 visual guide 中的：
    十字 / 扰动 3：单臂长度变化
    改变参数：水平臂 x span（竖直臂完全不动）
    建议范围：200 nm 到 600 nm
    建议步长：20 nm
    delta 定义：delta = horizontal_x_span - vertical_y_span
    降群路径：C4 -> C1

几何定义：
    - 水平臂对象：Si_cross_horizontal（x span 可变，y span = 160 nm 固定）
    - 竖直臂对象：Si_cross_vertical（完全不动，x span = 160 nm, y span = 580 nm）
    - 本扰动只改变 Si_cross_horizontal 的 x span，竖直臂所有参数保持不变。
"""
...
# ===== 单臂长度扫描参数 =====
# 水平臂 x span 从 SHORT 到 LONG 扫描
START_M = 200e-9
END_M = 700e-9
STEP_M = 25e-9
SINGLE_ARM_X_START_M = START_M
SINGLE_ARM_X_STOP_M = END_M
SINGLE_ARM_X_STEP_M = STEP_M
INCLUDE_EXACT_STOP_POINT = True

AUTO_CLIP = True
AUTO_STEP = True
TARGET_POINTS = 20
AUTO_STEP_MIN_M = 10e-9
AUTO_STEP_MAX_M = 50e-9
...
PERTURBATION_OUTPUT_DIR_NAME = "单臂长度变化扰动"
...
def effective_x_span_stop(vertical_y_span_m):
    actual_start = max(SINGLE_ARM_X_START_M, 50e-9)
    actual_stop = min(SINGLE_ARM_X_STOP_M, vertical_y_span_m * 1.5)
    if actual_start > actual_stop:
        raise ValueError("扫描范围无效。")
    return actual_start, actual_stop


def effective_x_span_step(actual_start, actual_stop):
    if not AUTO_STEP:
        if SINGLE_ARM_X_STEP_M <= 0: raise ValueError("步长必须大于 0。")
        return SINGLE_ARM_X_STEP_M
...
def build_sweep_points(horiz_x, horiz_y, horiz_z_min, horiz_z_max, vert_x, vert_y, vert_z_min, vert_z_max):
    actual_start, actual_stop = effective_x_span_stop(vert_y)
    step_m = effective_x_span_step(actual_start, actual_stop)
    ...
    points = []
    for global_index, x_span in enumerate(values):
        delta = x_span - vert_y
        points.append(SweepPoint(global_index, float(x_span), float(delta)))
    return points


def point_stem(point):
    return "{:04d}_Hx{:07.2f}nm_delta{:+08.2f}nm".format(
        point.global_index, nm(point.horizontal_x_span_m), nm(point.delta_m)
...
def set_single_arm_length(fdtd, point):
    """只改变水平臂 x span；竖直臂完全不动。"""
    fdtd.switchtolayout()
...
def save_abs2_plot(png_path, point, wavelength_m, transmission):
    ...
    ax.set_title("Single arm length: Hx={:.2f} nm, delta={:.2f} nm".format(nm(point.horizontal_x_span_m), nm(point.delta_m)))
...
def save_transmission_excel(xlsx_path, point, geometry, wavelength_m, frequency_hz, transmission):
    ...
    meta = [["item", "value"], ["perturbation", "single_arm_length"], ["horizontal_x_span_nm", nm(point.horizontal_x_span_m)],
            ["delta_nm", nm(point.delta_m)], ["delta_definition", "delta = Hx - Vy"]]
...
def write_scan_plan(plan_dir, points, geometry):
    ...
    with csv_path.open("w", newline="", encoding="utf-8-sig") as f:
        writer = csv.writer(f)
        writer.writerow(["global_index", "horizontal_x_span_nm", "delta_nm"])
        for point in points:
            writer.writerow([point.global_index, "{:.6f}".format(nm(point.horizontal_x_span_m)), "{:.6f}".format(nm(point.delta_m))])
    return csv_path
...
def write_structure_overview(run_dir, source_fsp, points, geometry):
    ...
    lines = ["# 十字单臂长度变化扰动结构状态说明", "", "## 母结构",
             ...
             "## 本批次扰动", "- 扰动名称：单臂长度变化",
             "- 实现方式：只改变水平臂 x span，竖直臂完全不动。",
             "- 降群路径：C4 -> C1", "- 计划仿真点数：{}".format(len(points))]
...
def write_manifest_header(manifest_path):
    with manifest_path.open("w", newline="", encoding="utf-8-sig") as f:
        writer = csv.writer(f); writer.writerow(["global_index", "horizontal_x_span_nm", "fsp_file", "excel_file", "png_file", "elapsed_s", "status"])


def append_manifest(manifest_path, point, paths, elapsed_s, status):
    with manifest_path.open("a", newline="", encoding="utf-8-sig") as f:
        writer = csv.writer(f)
        writer.writerow([point.global_index, "{:.6f}".format(nm(point.horizontal_x_span_m)), str(paths.fsp_file),
...
def parse_args():
    parser = argparse.ArgumentParser(description="Si 十字扰动 3：单臂长度变化 FDTD 自动化扫描")
...
def describe_point(point):
    parts = []
    for attr, label in (("horizontal_x_span_m", "Hx"), ("delta_m", "delta")):
...
def main():
    ...
    try:
        ...
        print("水平臂 x span: {:.3f} nm, 竖直臂 y span: {:.3f} nm".format(nm(horiz_x), nm(vert_y)))
        print("计划仿真点数: {}".format(len(points)))
        if mode == "preview":
            print("当前为预览模式。扫描点如下：")
            for point in points: print("  #{:04d}: Hx={:.2f} nm, delta={:.2f} nm".format(point.global_index, nm(point.horizontal_x_span_m), nm(point.delta_m)))
            return
        ...
        for index, point in enumerate(points, start=1):
            ...
            try:
                with lumapi.FDTD(hide=not args.show_gui) as fdtd:
                    ...
                    set_single_arm_length(fdtd, point); fdtd.save(str(work_fsp)); fdtd.run()
