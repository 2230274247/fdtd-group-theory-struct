# -*- coding: utf-8 -*-
"""
Si 十字扰动 2：横竖臂宽度差 FDTD 自动化扫描脚本
================================================

本脚本实现 visual guide 中的：
    十字 / 扰动 2：横竖臂宽度差
    改变参数：水平臂 y span（臂宽），竖直臂 x span（臂宽）固定
    建议范围：delta = -40 nm 到 +40 nm
    建议步长：5 nm
    delta 定义：delta = (horizontal_y_span - vertical_x_span) / 2
    降群路径：C4 -> C2

几何定义：
    - 水平臂对象：Si_cross_horizontal（x span = 580 nm 固定，y span 可变）
    - 竖直臂对象：Si_cross_vertical（x span = 160 nm 固定，y span = 580 nm 固定）
    - 本扰动只改变 Si_cross_horizontal 的 y span（水平臂的宽度），不改变长度。
"""
...
# ===== 横竖臂宽度差扫描参数 =====
# delta = (horizontal_y_span - vertical_x_span) / 2
# delta = 0 时两臂宽度相等（C4 对称）
START_M = -40e-9
END_M = 40e-9
STEP_M = 5e-9
ARM_WIDTH_DELTA_START_M = START_M
ARM_WIDTH_DELTA_STOP_M = END_M
ARM_WIDTH_DELTA_STEP_M = STEP_M
INCLUDE_EXACT_STOP_POINT = True

AUTO_CLIP_DELTA = True
MIN_ARM_WIDTH_M = 40e-9
AUTO_STEP = True
TARGET_POINTS = 15
AUTO_STEP_MIN_M = 2.5e-9
AUTO_STEP_MAX_M = 10e-9
...
PERTURBATION_OUTPUT_DIR_NAME = "横竖臂宽度差扰动"
...
class SweepPoint(NamedTuple):
    global_index: int
    delta_m: float
    horizontal_y_span_m: float
    vertical_x_span_m: float
...
def effective_delta_limits(vertical_x_span_m):
    delta_min = (MIN_ARM_WIDTH_M - vertical_x_span_m) / 2.0
    delta_max = (vertical_x_span_m * 2.0 - vertical_x_span_m) / 2.0
    return delta_min, delta_max


def effective_delta_stop(vertical_x_span_m):
    delta_min, delta_max = effective_delta_limits(vertical_x_span_m)
    actual_start = max(ARM_WIDTH_DELTA_START_M, delta_min)
    actual_stop = min(ARM_WIDTH_DELTA_STOP_M, delta_max)
    if actual_start > actual_stop:
        raise ValueError("delta 范围无效。")
    return actual_start, actual_stop


def effective_delta_step(actual_start, actual_stop):
    if not AUTO_STEP:
        if ARM_WIDTH_DELTA_STEP_M <= 0: raise ValueError("步长必须大于 0。")
        return ARM_WIDTH_DELTA_STEP_M
...
def import_lumapi():
    ...
    if not lumapi_file.exists(): raise FileNotFoundError("找不到 lumapi.py：{}".format(lumapi_file))
...
def build_delta_values(vertical_x_span_m):
    actual_start, actual_stop = effective_delta_stop(vertical_x_span_m)
...
def build_sweep_points(horiz_x, horiz_y, horiz_z_min, horiz_z_max, vert_x, vert_y, vert_z_min, vert_z_max):
    deltas = build_delta_values(vert_x)
    points = []
    for global_index, delta_m in enumerate(deltas):
        new_horiz_y = vert_x + 2.0 * delta_m
        points.append(SweepPoint(global_index, float(delta_m), float(new_horiz_y), float(vert_x)))
    return points


def point_stem(point):
    return "{:04d}_delta{:+08.2f}nm_Hy{:07.2f}nm".format(
        point.global_index, nm(point.delta_m), nm(point.horizontal_y_span_m)
...
def set_arm_width_diff(fdtd, point):
    """只改变水平臂的 y span（宽度）；竖直臂和臂长度保持不变。"""
    fdtd.switchtolayout()
    fdtd.select(HORIZONTAL_ARM_OBJECT_NAME)
    fdtd.set("y span", point.horizontal_y_span_m)
...
def save_abs2_plot(png_path, point, wavelength_m, transmission):
    ...
    ax.set_title("Arm width diff: delta={:.2f} nm, Hy={:.2f} nm".format(nm(point.delta_m), nm(point.horizontal_y_span_m)))
...
def save_transmission_excel(xlsx_path, point, geometry, wavelength_m, frequency_hz, transmission):
    ...
    meta = [["item", "value"], ["perturbation", "arm_width_diff"], ["delta_m", point.delta_m], ["delta_nm", nm(point.delta_m)],
            ["horizontal_y_span_nm", nm(point.horizontal_y_span_m)], ["vertical_x_span_nm", nm(point.vertical_x_span_m)],
            ["delta_definition", "delta = (Hy - Vx) / 2"]]
...
def write_scan_plan(plan_dir, points, geometry):
    ...
    with csv_path.open("w", newline="", encoding="utf-8-sig") as f:
        writer = csv.writer(f)
        writer.writerow(["global_index", "delta_nm", "horizontal_y_span_nm", "vertical_x_span_nm"])
        for point in points:
            writer.writerow([point.global_index, "{:.6f}".format(nm(point.delta_m)),
                            "{:.6f}".format(nm(point.horizontal_y_span_m)), "{:.6f}".format(nm(point.vertical_x_span_m))])
    return csv_path
...
def write_structure_overview(run_dir, source_fsp, points, geometry):
    ...
    lines = ["# 十字横竖臂宽度差扰动结构状态说明", "",
             "## 母结构", "- 母版 FSP：`{}`".format(source_fsp), "- 结构类型：C4对称结构 / 十字", "",
             "## 从 .fsp 读取到的固定几何",
             ...
             "## 本批次扰动", "- 扰动名称：横竖臂宽度差",
             "- 实现方式：只改变水平臂 y span（宽度），竖直臂和臂长度不变。",
             "- delta 定义：`(horizontal_y_span - vertical_x_span) / 2`",
             "- 降群路径：C4 -> C2", "- 计划仿真点数：{}".format(len(points))]
...
def write_manifest_header(manifest_path):
    with manifest_path.open("w", newline="", encoding="utf-8-sig") as f:
        writer = csv.writer(f)
        writer.writerow(["global_index", "delta_nm", "horizontal_y_span_nm", "fsp_file", "excel_file", "png_file", "elapsed_s", "status"])
...
def append_manifest(manifest_path, point, paths, elapsed_s, status):
    with manifest_path.open("a", newline="", encoding="utf-8-sig") as f:
        writer = csv.writer(f)
        writer.writerow([point.global_index, "{:.6f}".format(nm(point.delta_m)),
                        "{:.6f}".format(nm(point.horizontal_y_span_m)), str(paths.fsp_file),
...
def parse_args():
    parser = argparse.ArgumentParser(description="Si 十字扰动 2：横竖臂宽度差 FDTD 自动化扫描")
...
def apply_run_mode(args):
    ...
    if mode == "ask":
        ...
        print("请选择十字横竖臂宽度差脚本运行模式：")
        print("  1 = 测试模式：真实仿真前 {} 个点".format(TEST_POINT_COUNT))
        print("  2 = 完整真实仿真"); print("  3 = 预览模式：只生成扫描计划，不仿真")
...
def describe_point(point):
    parts = []
    for attr, label in (("delta_m", "delta"), ("horizontal_y_span_m", "Hy")):
        if hasattr(point, attr): parts.append("{}={:.3f} nm".format(label, nm(getattr(point, attr))))
...
def main():
    ...
    try:
        ...
        print("水平臂 y span (臂宽): {:.3f} nm, 竖直臂 x span (臂宽): {:.3f} nm".format(nm(horiz_y), nm(vert_x)))
        print("计划仿真点数: {}".format(len(points)))
        if mode == "preview":
            print("当前为预览模式。扫描点如下：")
            for point in points: print("  #{:04d}: delta={:.2f} nm, Hy={:.2f} nm".format(point.global_index, nm(point.delta_m), nm(point.horizontal_y_span_m)))
            return
        ...
        for index, point in enumerate(points, start=1):
            ...
            try:
                with lumapi.FDTD(hide=not args.show_gui) as fdtd:
                    ...
                    set_arm_width_diff(fdtd, point); fdtd.save(str(work_fsp)); fdtd.run()
