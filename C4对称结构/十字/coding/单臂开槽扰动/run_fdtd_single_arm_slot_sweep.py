# -*- coding: utf-8 -*-
"""
Si 十字扰动 4：单臂开槽 FDTD 自动化扫描脚本
================================================

本脚本实现 visual guide 中的：
    十字 / 扰动 4：单臂开槽
    改变参数：水平臂上的空气槽深度 slot_depth
    建议范围：0 nm 到 200 nm
    建议步长：10 nm
    delta 定义：delta = slot_depth
    降群路径：C4 -> C1 (or Cs)

几何定义：
    - 水平臂对象：Si_cross_horizontal（x span = 580 nm, y span = 160 nm）
    - 竖直臂对象：Si_cross_vertical（x span = 160 nm, y span = 580 nm）
    - 本扰动在水平臂的 +x 端添加一个空气矩形（etch），模拟开槽。
    - 槽对象名：air_slot（动态添加）
    - 槽宽度 = 水平臂 y span（贯穿臂宽）
    - 槽深度从 0 逐渐增大，从水平臂 +x 端向中心侵蚀。
    - slot_depth = 0 时无槽，作为基线结构。
"""
...
# ===== 单臂开槽扫描参数 =====
# slot_depth = 空气槽从水平臂 +x 端向中心侵蚀的深度
START_M = 0e-9
END_M = 200e-9
STEP_M = 10e-9
SLOT_DEPTH_START_M = START_M
SLOT_DEPTH_STOP_M = END_M
SLOT_DEPTH_STEP_M = STEP_M
INCLUDE_EXACT_STOP_POINT = True

# 槽的宽度方向：默认贯穿整个臂宽
SLOT_WIDTH_FRACTION = 1.0  # 1.0 = 贯穿整个臂宽

# 槽的位置：在水平臂的 +x 端
# 槽的 x 中心 = horizontal_x_span/2 - slot_depth/2
# 槽的 x span = slot_depth

AUTO_CLIP = True
MAX_SLOT_DEPTH_FRACTION = 0.45  # 槽深度不超过臂长的 45%
AUTO_STEP = True
TARGET_POINTS = 20
...
PERTURBATION_OUTPUT_DIR_NAME = "单臂开槽扰动"
...
def nm(value_m): return value_m * 1e9
def clamp(value, lower, upper): return max(lower, min(upper, value))


def effective_slot_depth_stop(horizontal_x_span_m):
    max_depth = horizontal_x_span_m * MAX_SLOT_DEPTH_FRACTION
    actual_start = SLOT_DEPTH_START_M
    actual_stop = min(SLOT_DEPTH_STOP_M, max_depth)
    if AUTO_CLIP:
        if actual_start > actual_stop:
            raise ValueError("槽深度范围无效。")
        return actual_start, actual_stop
    if SLOT_DEPTH_STOP_M > max_depth:
        raise ValueError("SLOT_DEPTH_STOP_M 超出安全上限。")
    return SLOT_DEPTH_START_M, SLOT_DEPTH_STOP_M


def effective_slot_depth_step(actual_start, actual_stop):
    if not AUTO_STEP:
        if SLOT_DEPTH_STEP_M <= 0: raise ValueError("步长必须大于 0。")
        return SLOT_DEPTH_STEP_M
...
def import_lumapi():
    ...
    for path in (api_dir, bin_dir):
        if hasattr(os, "add_dll_directory"): os.add_dll_directory(str(path))
        os.environ["PATH"] = str(path) + os.pathsep + os.environ.get("PATH", "")
    lumapi_file = api_dir / "lumapi.py"
    if not lumapi_file.exists(): raise FileNotFoundError("找不到 lumapi.py。")
    spec = importlib.util.spec_from_file_location("lumapi", str(lumapi_file))
    if spec is None or spec.loader is None: raise RuntimeError("无法导入 lumapi。")
...
def find_source_fsp():
    ...
    if len(fsp_files) != 1: raise RuntimeError("期望在 {} 中找到 1 个 .fsp。".format(SOURCE_FSP_DIR))
    return fsp_files[0]
...
def build_sweep_points(horiz_x, horiz_y, horiz_z_min, horiz_z_max, vert_x, vert_y, vert_z_min, vert_z_max):
    actual_start, actual_stop = effective_slot_depth_stop(horiz_x)
    step_m = effective_slot_depth_step(actual_start, actual_stop)
    if step_m <= 0: raise ValueError("步长必须大于 0。")
    values = []
    ...
    points = []
    for global_index, depth in enumerate(values):
        points.append(SweepPoint(global_index, float(depth)))
    return points


def point_stem(point):
    return "{:04d}_slot{:07.2f}nm".format(point.global_index, nm(point.slot_depth_m)).replace(".", "d")
...
def prepare_output_root(script_dir, resume, explicit_run_dir, run_mode="run"):
    ...
    if explicit_run_dir:
        ...
        if not run_dir.is_absolute(): run_dir = root / explicit_run_dir
        run_dir.mkdir(parents=True, exist_ok=True)
        return run_dir
    if resume:
        candidates = sorted(p for p in root.glob(RUN_FOLDER_PREFIX + "*") if p.is_dir())
        if not candidates: raise RuntimeError("没有可继续的批次。")
        return candidates[-1]
...
def prepare_output_folders(run_dir):
    folders = {"plan": run_dir / "00_scan_plan", "fsp": run_dir / "01_fsp_files",
               "excel": run_dir / "02_transmission_excel", "png": run_dir / "03_transmission_png_abs2",
               "logs": run_dir / "04_logs", "work": run_dir / WORK_FSP_DIR_NAME}
...
def paths_for_point(folders, point):
    ...
    return OutputPaths(folders["fsp"] / (stem + ".fsp"), folders["excel"] / (stem + ".xlsx"),
                       folders["png"] / (stem + ".png"))


def set_slot_on_arm(fdtd, point, geometry):
    """在水平臂 +x 端添加或修改空气槽。slot_depth = 0 时删除槽对象。"""
    horiz_x, horiz_y, horiz_z_min, horiz_z_max, vert_x, vert_y, vert_z_min, vert_z_max = geometry
    fdtd.switchtolayout()

    # 先删除已有的 air_slot 对象（如果存在）
    try:
        fdtd.select(SLOT_OBJECT_NAME)
        fdtd.delete()
    except Exception:
        pass

    if point.slot_depth_m <= 1e-18:
        # slot_depth = 0，不添加槽，保持原始结构
        return

    # 槽的几何参数：
    #   水平臂从 x = -horiz_x/2 到 x = +horiz_x/2
    #   槽在 +x 端：x_center = horiz_x/2 - slot_depth/2
    #   槽的 x span = slot_depth
    #   槽的 y span = horiz_y * SLOT_WIDTH_FRACTION（贯穿臂宽）
    #   槽的 z 范围 = 与水平臂相同（或略大以确保布尔减）
    slot_x_center = horiz_x / 2.0 - point.slot_depth_m / 2.0
    slot_x_span = point.slot_depth_m
    slot_y_span = horiz_y * SLOT_WIDTH_FRACTION
    # z 范围需要略大于 Si 结构以确保 etch 生效
    slot_z_min = horiz_z_min - 1e-9
    slot_z_max = horiz_z_max + 1e-9

    fdtd.addrect()
    fdtd.set("name", SLOT_OBJECT_NAME)
    fdtd.set("x", slot_x_center)
    fdtd.set("y", 0)
    fdtd.set("x span", slot_x_span)
    fdtd.set("y span", slot_y_span)
    fdtd.set("z min", slot_z_min)
    fdtd.set("z max", slot_z_max)
    fdtd.set("material", "etch")
    fdtd.set("override mesh order from material database", 1)
    fdtd.set("mesh order", 1)


def squeeze_array(value): return np.asarray(value).squeeze()
...
def read_transmission(fdtd):
    ...
    if wavelength is None:
        try: wavelength = fdtd.getdata(TRANSMISSION_MONITOR_NAME, "lambda")
        except: wavelength = None
    if frequency is None:
        try: frequency = fdtd.getdata(TRANSMISSION_MONITOR_NAME, "f")
        except: frequency = None
    if transmission is None:
        try: transmission = fdtd.transmission(TRANSMISSION_MONITOR_NAME)
        except: transmission = None
    if transmission is None: raise RuntimeError("无法读取透射谱 T。")
    ...
    if wavelength is None and frequency is None:
        ...
        wavelength = x_axis; frequency = x_axis
    elif wavelength is None:
        frequency = squeeze_array(frequency).astype(float); wavelength = 299792458.0 / frequency
    elif frequency is None:
        wavelength = squeeze_array(wavelength).astype(float); frequency = 299792458.0 / wavelength
    else:
        wavelength = squeeze_array(wavelength).astype(float); frequency = squeeze_array(frequency).astype(float)
    wavelength = np.asarray(wavelength).reshape(-1); frequency = np.asarray(frequency).reshape(-1)
...
def save_abs2_plot(png_path, point, wavelength_m, transmission):
    ...
    wavelength_nm = wavelength_m * 1e9; t_abs2 = np.abs(transmission) ** 2
    fig, ax = plt.subplots(figsize=(8.6, 5.2), dpi=180)
    ax.plot(wavelength_nm, t_abs2, linewidth=1.8, color="#1f77b4")
    ax.set_xlabel("Wavelength (nm)"); ax.set_ylabel("|T|^2")
    ax.set_title("Single arm slot: depth={:.2f} nm".format(nm(point.slot_depth_m)))
    ax.grid(True, alpha=0.28); fig.tight_layout(); fig.savefig(str(png_path)); plt.close(fig)


def xlsx_col_name(index):
    name = ""; index += 1
    while index: index, rem = divmod(index - 1, 26); name = chr(65 + rem) + name
    return name


def xlsx_cell(row_index, col_index, value):
    ref = "{}{}".format(xlsx_col_name(col_index), row_index + 1)
    if value is None: return '<c r="{}"/>'.format(ref)
    if isinstance(value, (int, float, np.integer, np.floating)): return '<c r="{}"><v>{:.16g}</v></c>'.format(ref, float(value))
...
def save_xlsx(xlsx_path, sheets):
    wb_sheets = []; rels = []; overrides = []
    for idx, sheet in enumerate(sheets, start=1):
        name = escape(sheet[0])
        wb_sheets.append('<sheet name="{}" sheetId="{}" r:id="rId{}"/>'.format(name, idx, idx))
    ...
    with zipfile.ZipFile(str(xlsx_path), "w", zipfile.ZIP_DEFLATED) as zf:
        zf.writestr("[Content_Types].xml", ct_xml); zf.writestr("_rels/.rels", root_rels)
        zf.writestr("xl/workbook.xml", wb_xml); zf.writestr("xl/_rels/workbook.xml.rels", rels_xml)
        for idx, sheet in enumerate(sheets, start=1): zf.writestr("xl/worksheets/sheet{}.xml".format(idx), xlsx_sheet_xml(sheet[1]))
...
def save_transmission_excel(xlsx_path, point, geometry, wavelength_m, frequency_hz, transmission):
    ...
    slot_x_center = horiz_x / 2.0 - point.slot_depth_m / 2.0 if point.slot_depth_m > 0 else 0
    meta = [["item", "value"], ["perturbation", "single_arm_slot"],
            ["slot_depth_nm", nm(point.slot_depth_m)], ["slot_width_nm", nm(horiz_y * SLOT_WIDTH_FRACTION)],
            ["slot_x_center_nm", nm(slot_x_center)],
            ["slot_position", "horizontal_arm_+x_end"],
            ["slot_material", "etch"]]
    save_xlsx(xlsx_path, [("transmission_abs2", data_rows), ("metadata", meta)])


def write_scan_plan(plan_dir, points, geometry):
    horiz_x, horiz_y = geometry[0], geometry[1]
    ...
    with csv_path.open("w", newline="", encoding="utf-8-sig") as f:
        ...
        for point in points:
            slot_x_center = horiz_x / 2.0 - point.slot_depth_m / 2.0 if point.slot_depth_m > 0 else 0
            writer.writerow([point.global_index, "{:.6f}".format(nm(point.slot_depth_m)),
                            "{:.6f}".format(nm(horiz_y * SLOT_WIDTH_FRACTION)),
                            "{:.6f}".format(nm(slot_x_center))])
    return csv_path
...
def write_structure_overview(run_dir, source_fsp, points, geometry):
    ...
    lines = ["# 十字单臂开槽扰动结构状态说明", "", "## 母结构",
             "- 母版 FSP：`{}`".format(source_fsp), "- 结构类型：C4对称结构 / 十字", "",
             "## 从 .fsp 读取到的固定几何",
             "- 水平臂 x span：{:.6f} nm".format(nm(horiz_x)),
             "- 水平臂 y span：{:.6f} nm".format(nm(horiz_y)),
             "- 竖直臂 x span：{:.6f} nm".format(nm(vert_x)),
             "- 竖直臂 y span：{:.6f} nm".format(nm(vert_y)), "",
             "## 本批次扰动", "- 扰动名称：单臂开槽",
             "- 实现方式：在水平臂 +x 端添加空气矩形（etch），模拟开槽。",
             "- 槽宽度：{:.6f} nm（贯穿臂宽）".format(nm(horiz_y * SLOT_WIDTH_FRACTION)),
             "- 槽深度范围：{:.6f} nm 到 {:.6f} nm".format(nm(SLOT_DEPTH_START_M), nm(SLOT_DEPTH_STOP_M)),
             "- 降群路径：C4 -> C1/Cs", "- 计划仿真点数：{}".format(len(points))]
    with doc_path.open("w", encoding="utf-8-sig") as f: f.write("\n".join(lines))
    return doc_path


def write_manifest_header(manifest_path):
    with manifest_path.open("w", newline="", encoding="utf-8-sig") as f:
        writer = csv.writer(f); writer.writerow(["global_index", "slot_depth_nm", "fsp_file", "excel_file", "png_file", "elapsed_s", "status"])


def append_manifest(manifest_path, point, paths, elapsed_s, status):
    with manifest_path.open("a", newline="", encoding="utf-8-sig") as f:
        writer = csv.writer(f)
        writer.writerow([point.global_index, "{:.6f}".format(nm(point.slot_depth_m)), str(paths.fsp_file),
                        str(paths.excel_file), str(paths.png_file), "{:.3f}".format(elapsed_s), status])


class TeeLogger(object):
    def __init__(self, log_path): self.terminal = sys.stdout; self.log = log_path.open("a", encoding="utf-8")
...
def parse_args():
    parser = argparse.ArgumentParser(description="Si 十字扰动 4：单臂开槽 FDTD 自动化扫描")
    parser.add_argument("--preview", action="store_true"); parser.add_argument("--test-run", action="store_true")
    parser.add_argument("--full-run", action="store_true"); parser.add_argument("--resume", action="store_true")
    parser.add_argument("--run-dir", default=None); parser.add_argument("--show-gui", action="store_true")
...
def apply_run_mode(args):
    ...
    if mode == "ask":
        ...
        print("请选择十字单臂开槽脚本运行模式：")
        print("  1 = 测试模式：真实仿真前 {} 个点".format(TEST_POINT_COUNT))
        print("  2 = 完整真实仿真"); print("  3 = 预览模式")
        choice = input("请输入 1/2/3 后按回车：").strip()
        if choice == "1": return "test"
        if choice == "2": return "full"
        if choice == "3": return "preview"
        raise RuntimeError("输入无效。")
    if mode not in ("test", "full", "preview"): raise RuntimeError('RUN_MODE 无效。')
    return mode


def point_outputs_exist(paths): return paths.fsp_file.exists() and paths.excel_file.exists() and paths.png_file.exists()
def format_duration(seconds):
    seconds = max(0.0, float(seconds))
    if seconds < 60: return "{:.1f} s".format(seconds)
    minutes = int(seconds // 60); sec = int(seconds % 60)
...
def describe_point(point):
    return "slot_depth={:.3f} nm".format(nm(point.slot_depth_m))
...
def print_runtime_progress(done_count, total_count, elapsed_s, run_started_at):
    ...
    print("    单次仿真时间: {}；已完成: {}/{}；还剩: {} 组；预计还需要: {}".format(
        format_duration(elapsed_s), done_count, total_count, remaining_count, format_duration(avg_s * remaining_count)))
...
def main():
    ...
    if SUPPRESS_NONCRITICAL_WARNINGS: warnings.filterwarnings("ignore", message=".*deprecated.*")
    ...
    log_path = folders["logs"] / "automation_run.log"; manifest_path = folders["logs"] / "manifest.csv"
    logger = TeeLogger(log_path); old_stdout = sys.stdout; sys.stdout = logger
    try:
        source_fsp = find_source_fsp()
        master_template_fsp = prepare_ascii_master_template(source_fsp, folders["work"])
        print("源 FSP: {}".format(source_fsp)); print("输出批次目录: {}".format(run_dir))
        lumapi = import_lumapi()
        with lumapi.FDTD(hide=not args.show_gui) as fdtd:
            fdtd.load(str(master_template_fsp)); geometry = read_cross_geometry(fdtd)
        horiz_x, horiz_y, horiz_z_min, horiz_z_max, vert_x, vert_y, vert_z_min, vert_z_max = geometry
        points = build_sweep_points(*geometry)
        if args.max_points is not None: points = points[: max(0, args.max_points)]
        if mode == "test": points = points[:TEST_POINT_COUNT]
        write_scan_plan(folders["plan"], points, geometry)
        write_structure_overview(run_dir, source_fsp, points, geometry)
        print("水平臂: {:.3f} x {:.3f} nm".format(nm(horiz_x), nm(horiz_y)))
        print("槽深度范围: {:.3f} 到 {:.3f} nm".format(nm(SLOT_DEPTH_START_M), nm(SLOT_DEPTH_STOP_M)))
        print("计划仿真点数: {}".format(len(points)))
        if mode == "preview":
            print("当前为预览模式。扫描点如下：")
            for point in points:
                slot_x_center = horiz_x / 2.0 - point.slot_depth_m / 2.0 if point.slot_depth_m > 0 else 0
                print("  #{:04d}: depth={:.2f} nm, x_center={:.2f} nm".format(point.global_index, nm(point.slot_depth_m), nm(slot_x_center)))
            return
        if not args.resume or not manifest_path.exists(): write_manifest_header(manifest_path)
        run_started_at = time.time()
        for index, point in enumerate(points, start=1):
            paths = paths_for_point(folders, point)
            if args.resume and point_outputs_exist(paths):
                print("[{}/{}] 已存在，跳过。".format(index, len(points))); continue
            print("[{}/{}] 开始仿真：{}".format(index, len(points), point_stem(point)))
            print("    当前参数: {}；剩余: {}/{}".format(describe_point(point), len(points) - index + 1, len(points)))
            start = time.time(); work_fsp = make_point_working_copy(master_template_fsp, point, folders["work"])
            try:
                with lumapi.FDTD(hide=not args.show_gui) as fdtd:
                    fdtd.load(str(work_fsp)); fdtd.switchtolayout()
                    apply_fdtd_runtime_settings(fdtd)
                    set_slot_on_arm(fdtd, point, geometry)
                    fdtd.save(str(work_fsp)); fdtd.run()
                    wavelength_m, frequency_hz, transmission = read_transmission(fdtd); fdtd.save(str(work_fsp))
                ...
                elapsed_s = time.time() - start; append_manifest(manifest_path, point, paths, elapsed_s, "ok")
                print("    完成，用时 {:.1f} s".format(elapsed_s))
                print_runtime_progress(index, len(points), elapsed_s, run_started_at)
            except Exception as exc:
                elapsed_s = time.time() - start; append_manifest(manifest_path, point, paths, elapsed_s, "failed: {}".format(exc))
                print("    失败：{}".format(exc)); raise
        print("全部完成。结果目录: {}".format(run_dir))
    finally:
        sys.stdout = old_stdout; logger.close()
