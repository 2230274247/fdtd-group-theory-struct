# -*- coding: utf-8 -*-
"""
Si 方环扰动 4：内孔尺寸扫描 FDTD 自动化扫描脚本
================================================

本脚本实现 visual guide 中的：
    方环 / 扰动 4：内孔尺寸扫描
    改变参数：内孔边长 inner_span（x span = y span 同步变化）
    建议范围：100 nm 到 400 nm
    建议步长：25 nm
    delta 定义：delta = (inner_span - inner_span_ref) / inner_span_ref
    降群路径：C4 -> C4（保持对称性，只改变模式体积）

几何定义：
    - 母结构外框对象：Si_outer_square
    - 内孔对象：air_inner_square
    - 本扰动只改变 air_inner_square 的 x span 和 y span（保持正方形），不移动中心。
    - 外框尺寸、厚度和材料保持不变。
    - inner_span = 300 nm 时为母版基线结构。

运行流程：
    1. 从 SOURCE_FSP_DIR 中寻找唯一 .fsp 文件；
    2. 复制到本批次结果目录下的工作子目录，作为 master_template.fsp；
    3. 每个尺寸点都从 master_template.fsp 再复制一个 work_*.fsp；
    4. 只在该 work_*.fsp 上改变 air_inner_square 的 x span 和 y span；
...
# ===== 内孔尺寸扫描参数 =====
# inner_span = 内孔边长（x span = y span）
START_M = 100e-9
END_M = 400e-9
STEP_M = 25e-9
INNER_SIZE_START_M = START_M
INNER_SIZE_STOP_M = END_M
INNER_SIZE_STEP_M = STEP_M
INCLUDE_EXACT_STOP_POINT = True

# ===== 参数联动设置 =====
AUTO_CLIP_INNER_SIZE = True
...
PERTURBATION_OUTPUT_DIR_NAME = "内孔尺寸扫描扰动"
...
def effective_inner_size_limits(outer_x_span_m, outer_y_span_m):
    """根据外框尺寸和安全边距，计算内孔边长的安全范围。"""
    # 内孔不能超过外框 - 2 * (MIN_EDGE_WIDTH_M + clearance)
    max_inner = min(outer_x_span_m, outer_y_span_m) - 2.0 * (MIN_EDGE_WIDTH_M + INNER_HOLE_EDGE_CLEARANCE_M)
    min_inner = 2.0 * INNER_HOLE_EDGE_CLEARANCE_M  # 内孔不能太小以至于失去物理意义

    if max_inner <= min_inner:
        raise ValueError("外框尺寸或安全边距设置不合理，无法生成有效的内孔尺寸范围。")

    return min_inner, max_inner


def effective_inner_size_stop(outer_x_span_m, outer_y_span_m):
    """获取实际的内孔尺寸终点值。"""
    min_inner, max_inner = effective_inner_size_limits(outer_x_span_m, outer_y_span_m)

    actual_start = max(INNER_SIZE_START_M, min_inner)
    actual_stop = min(INNER_SIZE_STOP_M, max_inner)

    if AUTO_CLIP_INNER_SIZE:
        if actual_start > actual_stop:
            raise ValueError(
                "内孔尺寸范围无效：start={:.3f} nm > stop={:.3f} nm。".format(
                    nm(actual_start), nm(actual_stop)
                )
            )
        return actual_start, actual_stop

    if INNER_SIZE_START_M < min_inner or INNER_SIZE_STOP_M > max_inner:
        raise ValueError(
            "INNER_SIZE 范围 [{:.3f}, {:.3f}] nm 超出安全范围 [{:.3f}, {:.3f}] nm。".format(
                nm(INNER_SIZE_START_M), nm(INNER_SIZE_STOP_M), nm(min_inner), nm(max_inner)
            )
        )
    return INNER_SIZE_START_M, INNER_SIZE_STOP_M


def effective_inner_size_step(actual_start, actual_stop):
    """获取实际的内孔尺寸步长。"""
    if not AUTO_STEP:
        if INNER_SIZE_STEP_M <= 0:
            raise ValueError("INNER_SIZE_STEP_M 必须大于 0。")
        return INNER_SIZE_STEP_M
...
def build_inner_sizes(outer_x_span_m, outer_y_span_m):
    """生成内孔尺寸扫描值列表。"""
    actual_start, actual_stop = effective_inner_size_stop(outer_x_span_m, outer_y_span_m)
    step_m = effective_inner_size_step(actual_start, actual_stop)
...
def build_sweep_points(outer_x_span_m, outer_y_span_m):
    """构建扫描点列表。"""
    inner_sizes = build_inner_sizes(outer_x_span_m, outer_y_span_m)
    points = []  # type: List[SweepPoint]
    for global_index, inner_span_m in enumerate(inner_sizes):
        edge_width = (outer_x_span_m - inner_span_m) / 2.0
        points.append(SweepPoint(global_index, float(inner_span_m), float(edge_width)))
    return points


def point_stem(point):
    return "{:04d}_inner{:07.2f}nm_edge{:07.2f}nm".format(
        point.global_index,
        nm(point.inner_span_m),
        nm(point.edge_width_m),
...
def set_inner_hole_size(fdtd, point):
    """只改变内孔 air_inner_square 的 x span 和 y span；位置和外框保持不变。"""
    fdtd.switchtolayout()
    fdtd.select(INNER_HOLE_OBJECT_NAME)
    fdtd.set("x span", point.inner_span_m)
    fdtd.set("y span", point.inner_span_m)
...
def save_abs2_plot(png_path, point, wavelength_m, transmission):
    ...
    ax.set_ylabel("|T|^2")
    ax.set_title(
        "Inner hole size: span={:.2f} nm, edge={:.2f} nm".format(
            nm(point.inner_span_m), nm(point.edge_width_m)
        )
    )
...
def save_transmission_excel(xlsx_path, point, geometry, wavelength_m, frequency_hz, transmission):
    ...
    ref_inner_span = inner_x_span_m  # 母版内孔边长
    metadata_rows = [
        ["item", "value"],
        ["perturbation", "inner_hole_size"],
        ["method", "change_inner_square_span_xy"],
        ["group_path", "C4 -> C4 (preserved)"],
        ...
        ["outer_x_span_m", outer_x_span_m],
        ["outer_x_span_nm", nm(outer_x_span_m)],
        ["outer_y_span_m", outer_y_span_m],
        ["outer_y_span_nm", nm(outer_y_span_m)],
        ["inner_span_m", point.inner_span_m],
        ["inner_span_nm", nm(point.inner_span_m)],
        ["ref_inner_span_m", ref_inner_span],
        ["ref_inner_span_nm", nm(ref_inner_span)],
        ["edge_width_m", point.edge_width_m],
        ["edge_width_nm", nm(point.edge_width_m)],
        ["delta_definition", "delta = (inner_span - ref_inner_span) / ref_inner_span"],
        ["delta", (point.inner_span_m - ref_inner_span) / ref_inner_span if ref_inner_span else 0],
    ]
...
def write_scan_plan(plan_dir, points, geometry):
    outer_x_span_m, outer_y_span_m, outer_z_min_m, outer_z_max_m, inner_x_span_m, inner_y_span_m, inner_z_min_m, inner_z_max_m = geometry
    ref_inner_span = inner_x_span_m
    csv_path = plan_dir / "scan_points.csv"
    with csv_path.open("w", newline="", encoding="utf-8-sig") as f:
        writer = csv.writer(f)
        writer.writerow(["global_index", "inner_span_nm", "edge_width_nm", "delta=(inner-ref)/ref"])
        for point in points:
            writer.writerow([
                point.global_index,
                "{:.6f}".format(nm(point.inner_span_m)),
                "{:.6f}".format(nm(point.edge_width_m)),
                "{:.9f}".format((point.inner_span_m - ref_inner_span) / ref_inner_span if ref_inner_span else 0),
            ])
    ...
    with summary_path.open("w", encoding="utf-8-sig") as f:
        f.write("\n".join([
            ...
            "==================================",
            "扰动名称: 内孔尺寸扫描",
            "实现方式: 固定外框，改变 air_inner_square 的 x span 和 y span（保持正方形）",
            ...
            ".fsp 中内孔基准 x span: {:.6f} nm".format(nm(inner_x_span_m)),
            ".fsp 中内孔基准 y span: {:.6f} nm".format(nm(inner_y_span_m)),
            "内孔尺寸起点: {:.6f} nm".format(nm(INNER_SIZE_START_M)),
            "内孔尺寸终点: {:.6f} nm".format(nm(INNER_SIZE_STOP_M)),
            "实际步长: {:.6f} nm".format(nm(effective_inner_size_step(*effective_inner_size_stop(outer_x_span_m, outer_y_span_m)))),
            "自动联动: {}".format(AUTO_CLIP_INNER_SIZE),
...
def write_structure_overview(run_dir, source_fsp, points, geometry):
    ...
    min_point = points[0] if points else None
    lines = [
        "# 方环内孔尺寸扫描扰动结构状态说明",
        "",
        "## 母结构",
        ...
        "- 内孔基准 x span：{:.6f} nm".format(nm(inner_x_span_m)),
        "- 内孔基准 y span：{:.6f} nm".format(nm(inner_y_span_m)),
        "- 外框厚度：{:.6f} nm".format(nm(outer_z_max_m - outer_z_min_m)),
        "- 基线边宽：{:.6f} nm".format(nm((outer_x_span_m - inner_x_span_m) / 2.0)),
        "- 固定不变量：外框尺寸、内孔中心位置、厚度、材料、FDTD 区域、光源、监视器均不在本脚本中改变。",
        "",
        "## 本批次扰动",
        "- 扰动名称：内孔尺寸扫描",
        "- 实现方式：只改变内孔 `air_inner_square` 的 x span 和 y span（保持正方形，中心不动）。",
        "- 变量定义：`inner_span = 内孔边长`，`delta = (inner_span - ref) / ref`。",
        "- 扫描范围：{:.6f} nm 到 {:.6f} nm".format(
            nm(effective_inner_size_stop(outer_x_span_m, outer_y_span_m)[0]),
            nm(effective_inner_size_stop(outer_x_span_m, outer_y_span_m)[1])
        ),
        "- 实际扫描步长：{:.6f} nm".format(nm(effective_inner_size_step(*effective_inner_size_stop(outer_x_span_m, outer_y_span_m)))),
        "- 降群路径：C4 -> C4（保持对称性）",
        "- 计划仿真点数：{}".format(len(points)),
    ]
    if min_point is not None:
        lines.append("- 最小内孔：{:.6f} nm，边宽 {:.6f} nm".format(nm(min_point.inner_span_m), nm(min_point.edge_width_m)))
    if max_point is not None:
        lines.append("- 最大内孔：{:.6f} nm，边宽 {:.6f} nm".format(nm(max_point.inner_span_m), nm(max_point.edge_width_m)))
    lines.extend([
        "",
        "## 输出文件",
...
def write_manifest_header(manifest_path):
    with manifest_path.open("w", newline="", encoding="utf-8-sig") as f:
        writer = csv.writer(f)
        writer.writerow(["global_index", "inner_span_nm", "edge_width_nm",
...
def parse_args():
    parser = argparse.ArgumentParser(description="Si 方环扰动 4：内孔尺寸扫描 FDTD 自动化扫描")
...
def main():
    ...
    try:
        ...
        points = build_sweep_points(outer_x_span_m, outer_y_span_m)
        ...
        print("内孔基准 span: {:.3f} nm".format(nm(inner_x_span_m)))
        print("基线边宽: {:.3f} nm".format(nm((outer_x_span_m - inner_x_span_m) / 2.0)))
        print("内孔尺寸范围: {:.3f} nm 到 {:.3f} nm，实际步长 {:.3f} nm".format(
            nm(effective_inner_size_stop(outer_x_span_m, outer_y_span_m)[0]),
            nm(effective_inner_size_stop(outer_x_span_m, outer_y_span_m)[1]),
            nm(effective_inner_size_step(*effective_inner_size_stop(outer_x_span_m, outer_y_span_m))),
        ))
        ...
        if mode == "preview":
            ...
            for point in points:
                print("  #{:04d}: inner_span={:.2f} nm, edge_width={:.2f} nm".format(
                    point.global_index,
                    nm(point.inner_span_m),
                    nm(point.edge_width_m),
                ))
        ...
        for index, point in enumerate(points, start=1):
            ...
            try:
                with lumapi.FDTD(hide=not args.show_gui) as fdtd:
                    ...
                    set_inner_hole_size(fdtd, point)
