# -*- coding: utf-8 -*-
"""
Si 方环扰动 2：边宽差 FDTD 自动化扫描脚本
==========================================

本脚本实现 visual guide 中的：
    方环 / 扰动 2：边宽差
    改变参数：外框 x span 与 y span 不相等，即 outer_x_span != outer_y_span
    建议范围：delta = (outer_x_span - outer_y_span) / 2 = -50 nm 到 +50 nm
    建议步长：5 nm
...
几何定义：
    ...
    - 本扰动只改变 Si_outer_square 的 x span，不改变 y span、内孔尺寸和位置。
    - delta = 0 nm 时外框为正方形，作为基线结构（C4 对称）。
    - delta > 0 时 x 方向边宽大于 y 方向边宽（C2 对称）。

运行流程：
    1. 从 SOURCE_FSP_DIR 中寻找唯一 .fsp 文件；
    2. 复制到本批次结果目录下的工作子目录，作为 master_template.fsp；
    3. 每个边宽差点都从 master_template.fsp 再复制一个 work_*.fsp；
    4. 只在该 work_*.fsp 上改变 Si_outer_square 的 x span；
...
# 这是环境警告，不是仿真错误；提前过滤可以避免运行窗口被无关提示刷屏。
...
# ===== 边宽差扫描参数 =====
# delta = (outer_x_span - outer_y_span) / 2
# delta = 0 时为正方形外框（基线）
# delta > 0 时 x 方向边宽更大
# delta < 0 时 y 方向边宽更大
START_M = -50e-9
END_M = 50e-9
STEP_M = 5e-9
EDGE_WIDTH_DELTA_START_M = START_M
EDGE_WIDTH_DELTA_STOP_M = END_M
EDGE_WIDTH_DELTA_STEP_M = STEP_M
INCLUDE_EXACT_STOP_POINT = True

# ===== 参数联动设置 =====
# 自动限制 delta 范围，使外框不超出周期、边宽不为负
...
PERTURBATION_OUTPUT_DIR_NAME = "边宽差扰动"
WORK_FSP_DIR_NAME = "05_work_fsp"
SUPPRESS_NONCRITICAL_WARNINGS = True


def chinese_timestamp():
    """生成中文时间戳；不用 strftime 写中文，避免 Lumerical Python 3.6 的 locale 编码问题。"""
    now = datetime.now()
...
def effective_delta_limits(outer_x_span_m, outer_y_span_m, inner_x_span_m, inner_y_span_m):
    """根据周期、内孔尺寸和安全边距，计算 delta 的安全范围。"""
    # 外框 x span 不能超过 period - clearance（这里不直接知道 period，从 outer_y_span 推断）
    # 但更直接的限制是：边宽不能小于 MIN_EDGE_WIDTH_M
    # edge_width_x = (outer_x_span - inner_x_span) / 2
    # edge_width_y = (outer_y_span - inner_y_span) / 2
    # 当 delta > 0: outer_x_span = outer_y_span + 2*delta
    #   edge_width_x = (outer_y_span + 2*delta - inner_x_span) / 2
    #   要求 edge_width_x >= MIN_EDGE_WIDTH_M
    # 当 delta < 0: outer_x_span = outer_y_span + 2*delta (delta 为负)
    #   edge_width_y 不变，edge_width_x 变小
    #   要求 edge_width_x >= MIN_EDGE_WIDTH_M

    base_edge_width_x = (outer_x_span_m - inner_x_span_m) / 2.0
    base_edge_width_y = (outer_y_span_m - inner_y_span_m) / 2.0

    # delta_max: edge_width_x 不超过合理范围，edge_width_y 不小于 MIN_EDGE_WIDTH_M
    # edge_width_y = base_edge_width_y (不变，因为只改 outer_x_span)
    # edge_width_x = base_edge_width_x + delta
    # 要求: edge_width_x >= MIN_EDGE_WIDTH_M
    #   => delta >= MIN_EDGE_WIDTH_M - base_edge_width_x
    delta_min = MIN_EDGE_WIDTH_M - base_edge_width_x
    # 要求: outer_x_span 不能太大（这里用 outer_y_span 作为参考上限）
    # 一般让 delta 不超过 base_edge_width_x 的一半作为安全限制
    delta_max = base_edge_width_x * 0.8

    return delta_min, delta_max


def effective_delta_stop(outer_x_span_m, outer_y_span_m, inner_x_span_m, inner_y_span_m):
    """获取实际的 delta 终点值。"""
    delta_min, delta_max = effective_delta_limits(outer_x_span_m, outer_y_span_m, inner_x_span_m, inner_y_span_m)

    actual_start = max(EDGE_WIDTH_DELTA_START_M, delta_min)
    actual_stop = min(EDGE_WIDTH_DELTA_STOP_M, delta_max)

    if actual_start > actual_stop:
        raise ValueError(
            "delta 范围无效：start={:.3f} nm > stop={:.3f} nm。"
            "请检查 EDGE_WIDTH_DELTA_START_M 和 EDGE_WIDTH_DELTA_STOP_M。".format(
                nm(actual_start), nm(actual_stop)
            )
        )
...
def effective_delta_step(actual_start, actual_stop):
    """获取实际的 delta 步长。"""
    if not AUTO_STEP:
        if EDGE_WIDTH_DELTA_STEP_M <= 0:
            raise ValueError("EDGE_WIDTH_DELTA_STEP_M 必须大于 0。")
        return EDGE_WIDTH_DELTA_STEP_M
...
def build_delta_values(outer_x_span_m, outer_y_span_m, inner_x_span_m, inner_y_span_m):
    """生成 delta 扫描值列表。"""
    actual_start, actual_stop = effective_delta_stop(outer_x_span_m, outer_y_span_m, inner_x_span_m, inner_y_span_m)
...
def build_sweep_points(outer_x_span_m, outer_y_span_m, inner_x_span_m, inner_y_span_m):
    """构建扫描点列表。"""
    deltas = build_delta_values(outer_x_span_m, outer_y_span_m, inner_x_span_m, inner_y_span_m)
    points = []  # type: List[SweepPoint]
    for global_index, delta_m in enumerate(deltas):
        outer_x = outer_y_span_m + 2.0 * delta_m  # outer_x_span = base_y_span + 2*delta
        points.append(SweepPoint(global_index, float(delta_m), float(outer_x), float(outer_y_span_m)))
    return points


def point_stem(point):
    return "{:04d}_delta{:+08.2f}nm_Ox{:07.2f}nm_Oy{:07.2f}nm".format(
        point.global_index,
        nm(point.delta_m),
...
def set_outer_frame_x_span(fdtd, point):
    """只改变外框 Si_outer_square 的 x span；y span、内孔保持不变。"""
    fdtd.switchtolayout()
...
def save_abs2_plot(png_path, point, wavelength_m, transmission):
    ...
    ax.set_ylabel("|T|^2")
    ax.set_title(
        "Edge width diff: delta={:.2f} nm, Ox={:.2f} nm, Oy={:.2f} nm".format(
            nm(point.delta_m), nm(point.outer_x_span_m), nm(point.outer_y_span_m)
        )
    )
...
def save_transmission_excel(xlsx_path, point, geometry, wavelength_m, frequency_hz, transmission):
    ...
    base_edge_x = (outer_y_span_m - inner_x_span_m) / 2.0
    base_edge_y = (outer_y_span_m - inner_y_span_m) / 2.0
    metadata_rows = [
        ["item", "value"],
        ["perturbation", "edge_width_diff"],
        ["method", "change_outer_frame_x_span"],
        ["group_path", "C4 -> C2"],
        ...
        ["delta_definition", "delta = (outer_x_span - outer_y_span) / 2"],
        ["edge_width_x_nm", nm((point.outer_x_span_m - inner_x_span_m) / 2.0)],
        ["edge_width_y_nm", nm((point.outer_y_span_m - inner_y_span_m) / 2.0)],
    ]
...
def write_scan_plan(plan_dir, points, geometry):
    ...
    with csv_path.open("w", newline="", encoding="utf-8-sig") as f:
        writer = csv.writer(f)
        writer.writerow(["global_index", "delta_nm", "outer_x_span_nm", "outer_y_span_nm",
                         "edge_width_x_nm", "edge_width_y_nm", "delta_definition"])
        for point in points:
            edge_x = (point.outer_x_span_m - inner_x_span_m) / 2.0
            edge_y = (point.outer_y_span_m - inner_y_span_m) / 2.0
            writer.writerow([
                point.global_index,
                "{:.6f}".format(nm(point.delta_m)),
    ...
    with summary_path.open("w", encoding="utf-8-sig") as f:
        f.write("\n".join([
            ...
            "================================",
            "扰动名称: 边宽差",
            "实现方式: 固定内孔和外框 y span，改变外框 Si_outer_square 的 x span",
            ...
            ".fsp 中外框 z min/z max: {:.6f} / {:.6f} nm".format(nm(outer_z_min_m), nm(outer_z_max_m)),
            ".fsp 中内孔 z min/z max: {:.6f} / {:.6f} nm".format(nm(inner_z_min_m), nm(inner_z_max_m)),
            "delta 起点: {:.6f} nm".format(nm(EDGE_WIDTH_DELTA_START_M)),
            "delta 终点: {:.6f} nm".format(nm(EDGE_WIDTH_DELTA_STOP_M)),
            "实际 delta 步长: {:.6f} nm".format(nm(effective_delta_step(*effective_delta_stop(outer_x_span_m, outer_y_span_m, inner_x_span_m, inner_y_span_m)))),
            "自动联动: {}".format(AUTO_CLIP_DELTA),
...
def write_structure_overview(run_dir, source_fsp, points, geometry):
    ...
    lines = [
        ...
        "- 外框 z 范围：{:.6f} nm 到 {:.6f} nm".format(nm(outer_z_min_m), nm(outer_z_max_m)),
        "- 内孔 z 范围：{:.6f} nm 到 {:.6f} nm".format(nm(inner_z_min_m), nm(inner_z_max_m)),
        "- 基线 x 方向边宽：{:.6f} nm".format(nm((outer_x_span_m - inner_x_span_m) / 2.0)),
        "- 基线 y 方向边宽：{:.6f} nm".format(nm((outer_y_span_m - inner_y_span_m) / 2.0)),
        "- 固定不变量：内孔尺寸、内孔位置、外框 y span、厚度、材料、FDTD 区域、光源、监视器均不在本脚本中改变。",
        "",
        "## 本批次扰动",
        "- 扰动名称：边宽差",
        "- 实现方式：只改变外框 `Si_outer_square` 的 x span，保持 y span 和内孔不变。",
        "- 变量定义：`delta = (outer_x_span - outer_y_span) / 2`。",
        "- 扫描范围：{:.6f} nm 到 {:.6f} nm".format(
            nm(effective_delta_stop(outer_x_span_m, outer_y_span_m, inner_x_span_m, inner_y_span_m)[0]),
            nm(effective_delta_stop(outer_x_span_m, outer_y_span_m, inner_x_span_m, inner_y_span_m)[1])
        ),
        "- 实际扫描步长：{:.6f} nm".format(nm(effective_delta_step(*effective_delta_stop(outer_x_span_m, outer_y_span_m, inner_x_span_m, inner_y_span_m)))),
        "- 降群路径：C4 -> C2",
        "- 计划仿真点数：{}".format(len(points)),
    ]
    if max_point is not None:
        lines.extend([
            "- 最大扰动点：outer_x_span = {:.6f} nm，delta = {:.6f} nm".format(
                nm(max_point.outer_x_span_m), nm(max_point.delta_m)
            ),
...
def parse_args():
    parser = argparse.ArgumentParser(description="Si 方环扰动 2：边宽差 FDTD 自动化扫描")
    ...
    parser.add_argument("--run-dir", default=None, help="指定输出批次目录。相对路径会放在 results/边宽差扰动 下。")
    parser.add_argument("--show-gui", action="store_true", help="显示 FDTD 图形界面。默认隐藏界面。")
    parser.add_argument("--max-points", type=int, default=None, help="最多处理多少个扫描点；常用于临时测试。")
...
def main():
    ...
    try:
        ...
        if abs(outer_x_span_m - EXPECTED_OUTER_X_SPAN_M) > 2e-9:
            print("提醒：读取到的外框 x span {:.3f} nm 与参考值 {:.3f} nm 略有差异，后续以 .fsp 读取值为准。".format(nm(outer_x_span_m), nm(EXPECTED_OUTER_X_SPAN_M)))
        if abs(outer_y_span_m - EXPECTED_OUTER_Y_SPAN_M) > 2e-9:
            print("提醒：读取到的外框 y span {:.3f} nm 与参考值 {:.3f} nm 略有差异，后续以 .fsp 读取值为准。".format(nm(outer_y_span_m), nm(EXPECTED_OUTER_Y_SPAN_M)))
        ...
        print("x 方向边宽: {:.3f} nm".format(nm((outer_x_span_m - inner_x_span_m) / 2.0)))
        print("y 方向边宽: {:.3f} nm".format(nm((outer_y_span_m - inner_y_span_m) / 2.0)))
        ...
        if mode == "preview":
            ...
            for point in points:
                print("  #{:04d}: delta={:.2f} nm, Ox={:.2f} nm, Oy={:.2f} nm".format(
                    point.global_index,
                    nm(point.delta_m),
                    nm(point.outer_x_span_m),
                    nm(point.outer_y_span_m),
        ...
        for index, point in enumerate(points, start=1):
            ...
            try:
                with lumapi.FDTD(hide=not args.show_gui) as fdtd:
                    ...
                    set_outer_frame_x_span(fdtd, point)
